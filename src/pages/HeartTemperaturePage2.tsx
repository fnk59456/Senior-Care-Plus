...
export default function HeartTemperaturePage2() {
  ...
  const handleHeartIconClick = () => {
    navigate('/heart-temp-monitoring', { state: { patientName } })
  }
  ...
  // 統一設備選擇（同時控制心率和體溫）
  const [allDevices, setAllDevices] = useState<CloudDevice[]>([])
    ...
    const updateMqttData = () => {
      ...
      try {
        ...
        formatted.forEach(data => {
          ...
          setSelectedDevice(prev => prev || data.MAC)
        })
  ...
  const filteredHeartRecords = useMemo(() => {
    let filtered = [...heartDeviceRecords.filter(r => selectedDevice ? r.MAC === selectedDevice : true)]
    ...
    return filtered
  }, [heartDeviceRecords, heartRecordFilter, heartTimeRange, selectedDevice])
  ...
  const filteredTempRecords = useMemo(() => {
    let filtered = [...tempDeviceRecords.filter(r => selectedDevice ? r.MAC === selectedDevice : true)]
    ...
    return filtered
  }, [tempDeviceRecords, tempRecordFilter, tempTimeRange, selectedDevice])
  ...
  // 合併設備列表（顯示所有設備，包含心率和體溫數據）
  const mergedDevices = useMemo(() => {
    const deviceMap = new Map<string, CloudDevice>()

    // 從心率記錄中獲取設備
    heartDeviceRecords.forEach(record => {
      if (!deviceMap.has(record.MAC)) {
        deviceMap.set(record.MAC, {
          MAC: record.MAC,
          ...
          topic: record.topic,
          topicGateway: record.topicGateway,
          residentId: record.residentId,
          residentName: record.residentName,
          residentRoom: record.residentRoom,
    ...
    // 從體溫記錄中獲取設備
    tempDeviceRecords.forEach(record => {
      const existing = deviceMap.get(record.MAC)
      if (existing) {
        existing.recordCount += 1
        if (record.datetime > existing.lastSeen) {
          existing.lastSeen = record.datetime
        }
      } else {
        deviceMap.set(record.MAC, {
          MAC: record.MAC,
    ...
    return Array.from(deviceMap.values()).sort((a, b) => b.lastSeen.getTime() - a.lastSeen.getTime())
  }, [heartDeviceRecords, tempDeviceRecords])
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div className="bg-pink-50 p-3 rounded-lg">
                ...
                <div className="text-2xl font-bold text-pink-600">{mergedDevices.length}</div>
              </div>
              <div className="bg-green-50 p-3 rounded-lg">
                <div className="font-medium text-green-800">{t("pages:heartRate.cloudDeviceMonitoring.totalRecords")}</div>
                <div className="text-2xl font-bold text-green-600">{heartDeviceRecords.length + tempDeviceRecords.length}</div>
              </div>
                  ...
                  {mergedDevices
                    .filter(device => {
                      if (selectedGateway) {
                        const gateway = gateways.find(gw => gw.id === selectedGateway)
                        if (gateway) {
                          const heartRecords = heartDeviceRecords.filter(record => record.MAC === device.MAC)
                          const tempRecords = tempDeviceRecords.filter(record => record.MAC === device.MAC)
                          const allRecords = [...heartRecords, ...tempRecords]
                    ...
                    .map(device => {
                      ...
                      return (
                        <button
                          key={device.MAC}
                          onClick={() => setSelectedDevice(device.MAC)}
      ...
      {/* 心率數據區域 */}
      {selectedDevice && heartDeviceRecords.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center">
              <Heart className="mr-2 h-5 w-5" />
              {t("pages:heartRate.deviceHeartRateData.title")} - {(() => {
                const device = mergedDevices.find(d => d.MAC === selectedDevice)
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="space-y-3 max-h-80 overflow-y-auto">
                {heartDeviceRecords.filter(record => record.MAC === selectedDevice).slice(0, 20).map((record, index) => (
      ...
      {/* 體溫數據區域 */}
      {selectedDevice && tempDeviceRecords.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="space-y-3 max-h-80 overflow-y-auto">
                {tempDeviceRecords.filter(record => record.MAC === selectedDevice).slice(0, 20).map((record, index) => (
                  <div key={index} className="flex items-center justify-between p-3 rounded-lg border">
                    <div className="flex items-center gap-3">
